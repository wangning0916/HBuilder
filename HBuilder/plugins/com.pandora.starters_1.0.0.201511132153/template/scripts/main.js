...
(function($, owner) {
    ...
    var ui = {
        ...
        oAccountItems: $('.ui-other-account li'),
        ...
        oAccountAlert: $('#OAccount .ui-alert'),
        settingsAlert: $('.ui-view-settings .ui-alert'),
        mask: $('.ui-mask'),
        allControls: $('input,botton,a'),
        inputs: $('input'),
        generalControls: $('.ui-control-general'),
        navControls: $('.ui-control-nav'),
        refreshBtn: $('.ui-refresh'),
        backBtn: $('.ui-back')
    };
    ...
    ui.forgetPassword.on('click', function(event) {
        ...
        invoke.forgetPassword(loginInfo, owner.forgetPasswordCallback);
    });

    ui.offlineLogin.on('click', function(event) {
        ui.mask.show();
        var loginInfo = {
            account: ui.loginAccountBox.val(),
            password: ui.loginPasswordBox.val()
        };
        owner.offlineLoginCallback = function(rs) {
            if (rs) {
                ui.loginAlert.html(rs);
            }
            ui.mask.hide();
        };
        invoke.offlineLogin(loginInfo, owner.offlineLoginCallback);
    });
    ...
    //第三方登录
    var oAccountViewIsShow = false;
    var showOAccountView = function() {
        ui.mask.show();
        if (window.isIE) {
            ui.groupView.fadeOut(300, function() {
                ui.oAccountView.fadeIn(300, function() {
                    ui.mask.hide();
                });
            });
        } else {
            transitions.change(ui.groupView, ui.oAccountView, 32, function() {
                ui.mask.hide();
            });
        }
        oAccountViewIsShow = true;
        //
        ui.generalControls.addClass('ui-control-hidden');
        ui.navControls.removeClass('ui-control-hidden');
        //setViewTabState(ui.oAccountView);
    };

    var hideOAccountView = function() {
        ui.mask.show();
        if (window.isIE) {
            ui.oAccountView.fadeOut(300, function() {
                ui.groupView.fadeIn(300, function() {
                    ui.mask.hide();
                });
            });
        } else {
            transitions.change(ui.oAccountView, ui.groupView, 33, function() {
                ui.mask.hide();
            });
        }
        oAccountViewIsShow = false;
        //
        ui.navControls.addClass('ui-control-hidden');
        ui.generalControls.removeClass('ui-control-hidden');
        //setViewTabState(ui.oAccountView);
    };
    ui.oAccountItems.on('click', function() {
        var item = this;
        var id = item.getAttribute('data-id');
        ui.oAccountAlert.html('<span class="ui-icon-loading"></span><span>正在转到第三方登录...</span>');
        owner.getOAccountLoginUrlCallback = function(url) {
            ui.oAccountView.off('load');
            ui.oAccountView.attr('src', '');
            ui.oAccountView.on('load', function() {
                showOAccountView();
                setTimeout(function() {
                    ui.oAccountAlert.html('');
                    ui.oAccountView.off('load');
                }, 500);
            });
            ui.oAccountView.attr('src', url);
        };
        invoke.getOAccountLoginUrl(id, owner.getOAccountLoginUrlCallback);
    });
    ui.backBtn.on('click', function() {
        hideOAccountView();
    });
    ui.refreshBtn.on('click', function() {
        var url = ui.oAccountView.attr('src');
        ui.oAccountView.attr('src', '');
        ui.oAccountView.attr('src', url);
    });
